...
def run_molgraphconv(config):
    ...
    if config["model"] == "GAT":
        model = dc.models.GATModel(n_tasks=1, mode="regression", batch_size=16, predictor_hidden_feats=512, tensorboard=True, model_dir=model_dir)
    elif config["model"] == "GCN":
        model = dc.models.GCNModel(n_tasks=1, mode="regression", batch_size=16, predictor_hidden_feats=512, tensorboard=True, model_dir=model_dir)
    elif config["model"] == "MPNN":
        model = dc.models.torch_models.MPNNModel(n_tasks=1, mode="regression", batch_size=16, predictor_hidden_feats=2048, tensorboard=True, model_dir=model_dir)
    elif config["model"] == "AttentiveFP":
        model = dc.models.AttentiveFPModel(n_tasks=1, mode="regression", batch_size=16, predictor_hidden_feats=512, tensorboard=True, model_dir=model_dir)
